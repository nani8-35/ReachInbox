...
async function start() { await migrate(db); await recoverUnqueuedEmails(); await ensureSearchIndex(); const app = express(); app.use(cors({ origin: config.frontendUrl, credentials: true })); app.use(express.json({ limit: "2mb" }));
