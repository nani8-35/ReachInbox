...
const scheduleSchema = z.object({ sender: emailAddress, subject: z.string().trim().min(1).max(255), body: z.string().trim().min(1), recipients: z.array(emailAddress).min(1).max(5_000), startAt: z.string().datetime(), delaySeconds: z.number().int().min(0).max(86_400) });
