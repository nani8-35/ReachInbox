...
function redirectGoogle() { const params = new URLSearchParams({ client_id: process.env.GOOGLE_CLIENT_ID ?? "", redirect_uri: process.env.GOOGLE_CALLBACK_URL ?? "", response_type: "code", scope: "openid email profile", access_type: "offline", prompt: "select_account" }); return `https://accounts.google.com/o/oauth2/v2/auth?${params}`; }
...
authRouter.get("/google/callback", async (req, res, next) => { try {
  ...
  res.setHeader("Set-Cookie", `${cookieName}=${jwt.sign(user, config.jwtSecret, { expiresIn: "7d" })}; Path=/; HttpOnly; SameSite=Lax`); res.redirect(config.frontendUrl);
} catch (error) { next(error); } });
authRouter.post("/logout", (_req, res) => { res.setHeader("Set-Cookie", `${cookieName}=; Path=/; HttpOnly; Max-Age=0`); res.status(204).end(); });
